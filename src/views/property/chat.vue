...
export default {
  ...
  methods: {
    ...
    createContent(remoteUser, nowUser, text) {  // 这个方法是用来将 json的聊天消息数据转换成 html的。
      let html
      console.log('当前用户发送的消息',text,nowUser)
      console.log('远程',text,remoteUser)

      // 当前用户消息
      if (nowUser) { // nowUser 表示是否显示当前用户发送的聊天消息，绿色气泡
        console.log('当前用户发送的消息=====',text)
    ...
    init() {
      this.user = localStorage.getItem("role") ? JSON.parse(localStorage.getItem("role")) : {}
      ...
      if (typeof (WebSocket) == "undefined") {
        ...
        console.log("您的浏览器支持WebSocket");
        let socketUrl = 'ws://'+location.host+'/socket/imserver/' + username;
        // socketUrl = socketUrl.replace("http", "ws").replace("http", "ws");
        console.log('=host=====',socketUrl)
        if (socket != null) {
          socket.close();
          socket = null;
        }
